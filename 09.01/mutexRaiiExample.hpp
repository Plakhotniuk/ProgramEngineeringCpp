...
Пример:
    Допустим мы хотим в многопоточной программе избежать гонки данных при работе с каким-то объектом.
    Тогда для безопасной работы можно спроектировать шаблонный RAII класс прозрачной мьютекс-обертки,
     который в конструкторе захватывает ресурс: мьютекс, и в деструкторе его отпускает.
 */
